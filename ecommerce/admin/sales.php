 ...
 -->                </thead>
                <tbody>
                  <?php
                    ...
                    try{
                      $stmt = $conn->prepare("SELECT *, sum(totalPrice) as total,  orders.id AS ordersid FROM orders LEFT JOIN users ON orders.user_id=users.id GROUP BY placed_on,user_id");
                      $stmt->execute();
                      foreach($stmt as $row){
